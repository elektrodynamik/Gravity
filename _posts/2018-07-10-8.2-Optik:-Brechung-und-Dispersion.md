...
Hierbei ist der erste Term die Frequenz $$ \omega_0$$, der zweiter Term die Gruppengeschwindigkeit $$ v_{gr.} $$ und der dritte Term $$ \gamma $$ der Dispersionparameter.

Einsetzen in $$ \psi(z,t) $$ ohne quadratischen Teil.:

$$ \psi(z,t) \approx \dfrac{1}{\sqrt{2\pi}} \int_{-\infty}^\infty {\rm d} k \hat{\psi}(k) e^{i[kz - \omega_0 t - v_{\rm gr}(k - k_0)$$ 