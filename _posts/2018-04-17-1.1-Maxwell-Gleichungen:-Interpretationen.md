...
$$ \vec E (\vec r ) \equiv \frac{ \vec F (\vec r) }{q_{\text{Test.}} } = \sum_i \frac{ q_T q_i}{q_T} \frac{ \vec r - \vec r'}{|\vec r - \vec r' |^3} = \int \rm d^2 \vec r' \rho(\vec r') \frac{\vec r - \vec r'}{| \vec r - \vec r'|} $$  für $$ \rho(\vec r') = \sum_i q_i \delta(\vec r - \vec r') $$
