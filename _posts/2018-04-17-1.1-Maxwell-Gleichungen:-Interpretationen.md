...
_Beispiel_: Glockenkurve: $$ \varphi(x) =  \frac{1}{ \sqrt{2 \pi t} } e^{ \frac{-(x-x_0)}{2t} }  $$ produziert $$ \langle f, \varphi \rangle \rightarrow_{t \to 0} f(x_0) $$

_Definition_ : $$ \delta (x) = \lim_{t \to 0} \frac{1}{\sqrt{2 \pi t}} e^{ \frac{-(x-x_0)}{2t} } $$, d.h. : $$ \int f(x) \delta(x-x_0) = f(x_0) $$
...
\delta(x-a) = -f(a) = \int \delta''(x-a) = + \int f''(x) = -f''(a) $$

_Beispiel_: System von $$N$$-Punktladungen $$q_i$$, $$ i \in \{1, \dots , N \}

$$ \vec E (\vec r ) \equiv \frac{\vec F(\vec r)}{q_{\text{Test.}} $$
