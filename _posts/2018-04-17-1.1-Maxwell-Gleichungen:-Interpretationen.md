...
--
**Interpretaion der MG (III): keine Monopole**

$$ \vec \nabla B = \Leftrightarrow \int \rm d \vec \sigma \vec B (\vec x, t) = 0 $$

(i) Es gibt keine Quellen|Senken von $$ \vec B $$ ('' Es gibt keine magn. Monopole'' )

(ii) $$ \vec B$$-Feldlinien bilden sich ind geschlossenen Kurven aus.

(iii) Durch $$ \vec \nabla \cdot $$ (II) folgt $$\vec \nabla \cdot ( \vec \nabla \wedge \vec E) + \partial_{ct} \vec \nabla \vec B = 0  \rightarrow \vec B (\vec x, t) = fkt.(\vec x) =^! 0$$ 

--

**Interpretation der MG(I
