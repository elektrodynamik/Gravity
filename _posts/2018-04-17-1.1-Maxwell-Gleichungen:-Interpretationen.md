...
$$ \vec E (\vec r ) \equiv \frac{ \vec F (\vec r) }{q_{\text{Test.}} } = \sum_i \frac{ q_T q_i}{q_T} \dfrac{ \vec r - \vec r'}{|\vec r - \vec r' |^3} = \int \rm d^2 \vec r' \rho(\vec r') \dfrac{\vec r - \vec r'}{| \vec r - \vec r'|} $$ für $$ \rho(\vec r') = \sum_i q_i \delta(\vec r - \vec r') $$

_Identitäten_:

- $$ \vec \nabla \dfrac{\vec r - \vec r'}{|\vec r - \vec r'|^3} = \dots = 4 \pi \delta(\vec r - \vec r')$$

- $$ - \vec \nabla \dfrac{1}{|\vec r - \vec r'|} = \dots = \dfrac{\vec r - \vec r'}{|\vec r - \vec r'|} $$ ( siehe Rechenmethoden)

- $$ \Rightarrow -\Delta \dfrac{1}{|\vec r - \vec r'|} = 4 \pi \delta(\vec r - \vec r') $$

- $$ \Rightarrow (-\Delta \phi) (\vec r) = 4 \pi \delta(\vec r) $$ wird gelöst durch $$ \phi(\vec r) = \dfrac{1}{4 \pi} \int \rm d^3 \vec r' \dfrac{\rho(\vec r}{|\vec r - \vec r'|^3} 
