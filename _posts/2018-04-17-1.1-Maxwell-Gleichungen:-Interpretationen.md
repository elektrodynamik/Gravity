...
- $$ \vec \nabla \dfrac{a}{|\vec r - \vec r' |} = \dots = \dfrac{\vec r - \vec r'}{|\vec r - \vec r'|} $$ ( siehe Rechenmethoden)
- $$ \Rightarrow -\Delta \dfrac{1}{|\vec r - \vec r'|} = 4 \pi \delta(\vec r - \vec r') $$
- $$ \Rightarrow (-\Delta \phi) (\vec r) = 4 \pi \delta(\vec r) $$ 

wird gelöst durch $$ \phi(\vec r) = \dfrac{1}{4 \pi} \int \rm d^3 \vec r' \dfrac{\rho(\vec r)}{|\vec r - \vec r'|^3} $$
