...
$$ \int_V {\rm d}^3 x' \{ - 4 \pi \phi(\vec x') \delta(\vec x -\vec x') + \dfrac{4 \pi \rho(\vec x')}{\mid \vec x - \vec x'} \} \\= \int_F {\rm d}^3 x' \{\phi(\vec x') \partial_{\vec n'} \dfrac{1}{\mid \vec x - \vec x'} - \dfrac{1}{\mid \vec x - \vec x'} \partial_{\vec n'} \phi(\vec x') \} \\ \Rightarrow$$
