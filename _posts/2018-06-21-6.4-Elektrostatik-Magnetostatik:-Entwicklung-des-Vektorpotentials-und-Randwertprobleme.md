...
Man kann nicht $$\partial_{\vec n'} G(\vec x,\vec x')\mid_{\vec x' \in F} \stackrel{!}{=} 0 $$ verlangen, da im Widerspruch zu $$ -\Delta' G(\vec x,\vec x') = -4\pi \delta(\vec x - \vec x') \Leftrightarrow \int_F {\rm d}^2 x \int_F {\rm d}^2 x' \partial_{\vec n'} G(\vec x, \vec x')$$