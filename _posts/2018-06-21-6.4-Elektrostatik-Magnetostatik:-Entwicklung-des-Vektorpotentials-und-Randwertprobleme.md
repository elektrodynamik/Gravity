...
$$ \Rightarrow \phi(\vec x) = \int_V {\rm d}^3 x' G(\vec x,\vec x') \rho(\vec x') + \dfrac{1}{4\pi} \int_F {\rm d}^2 x' G(\vec x,\vec x') \partial_{\vec n'} \phi(\vec x') + \langle \phi \rangle_F, \\ \langle \phi \rangle_F = \dfrac{1}{\mid F\mid}$$