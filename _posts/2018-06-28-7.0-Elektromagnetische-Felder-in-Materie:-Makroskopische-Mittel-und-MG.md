...
$$ \require{cancel} \langle \rho_g(\vec x,t) \rangle \\ = \sum_i \int {\rm d}^3 x' \rho^{(i)} (\vec x' - \vec x^{(i)} ) g(\vec x - \vec x') \\ = \sum_i \int {\rm d}^3 x'' \rho^{(i)} (\vec x'') g(\vec x - \vec x^{(i)} - \vec x'') \\  \stackrel{\rm (1)}{=} \sum_i g(\vec x - \vec x^{(i)} \int {\rm d}^3 x'' \rho^{(i)}(\vec x'')$$