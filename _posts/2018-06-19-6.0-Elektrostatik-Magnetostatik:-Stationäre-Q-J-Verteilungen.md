...
$$ \phi^{(1)}(\vec x) = \dfrac{1}{\mid \vec x\mid} \int {\rm d}^3 x' x' \rho(\vec x') \, \text{( elektrisches Dipolmoment) } $$

$$ \phi^{(2)}(\vec x) = \dfrac{\vec x}{\mid 2 \vec x\mid^5} \int {\rm d}^3 x_i' x_j' \rho(\vec x) \, \text{( Elektrisches Monopolmoment) } $$
