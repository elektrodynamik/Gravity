...
$$ \rho(\vec x) \sum q_i \delta (\vec x - \vec x^{(i)} \to E_{\rm em}  = \dfrac{1}{2} \sum_{i,j} \int {\rm d}^3 x {\rm d}^3 x' \dfrac{\delta(\vec x - \vec x^{(i)} \delta(\vec x - \vec x^{(j)}}{\mid \vec x -\vec x'\mid } = \dfrac{1}{2} \sum_{i,j} \dfrac{q_i q_j}{\mid \vec x^(i) - \vec x^{(j)}$$