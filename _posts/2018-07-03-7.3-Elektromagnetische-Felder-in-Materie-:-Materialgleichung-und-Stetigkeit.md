...
$$ = \int_{-\infty}^{\infty} \hat{\alpha}(\omega) \dfrac{1}{\sqrt{2\pi}} \int_{-\infty}^{\infty} {\rm d} \tau \vec E(\tau) e^{-i\omega t} e^{i\omega \tau} \\  \int_{-\infty}^{\infty} {\rm d} \omega \hat{\alpha(\omega)}$$
