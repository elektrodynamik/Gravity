...
$$ ( T^{\mu \nu} ) =  \begin{pmatrix} \dfrac{ \vec E^2 + \vec B^2}{8 \pi} & \dfrac{[\vec E \wedge \vec B ]^T}{4 \pi} \\  \dfrac{[\vec E \wedge \vec B ]}{4 \pi} & -(T_{ik} ) \end{pmatrix} =  \begin{pmatrix} \epsilon_{em} & c \vec g_{em}^T \\ c \vec g_em & - (T_{ik} ) \end{pmatrix}  $$ 

mit

$$ \epsilon_{em} = \frac{1}{8 \pi} (\vec E^2 + \vec B^2) $$ ' Energiedichte des Feldes '

$$ \vec g_{em} = \frac{1}{4 \pi c } (\vec E \wedge \vec B ) $$ ' Impulsdichte/ Energiestromdichte des Feldes'
...
$$ \leftrightarrow \\  \nu = 0 : \partial_t \epsilon_{mech.} + \partial_t \epsilon_{em} = - \vec \nabla \vec S \\ \nu = i: ( \partial_t g_{mech.} + \partial_{e.m.} )_ i = \partial_j T_{ij} $$

Spezialfall: Volumen ohne Ladungen.

$$ \partial_\mu T^{\mu \nu} = 0^{(\nu)} $$, d.h. $$ T^{0\nu} + \partial_i T^{i\nu} = 0 , \ \forall \nu $$

Intergral über Raum $$ \partial_{ct} \int {\rm d}^3 x T^{0\nu} = 0 $$ 