...
$$ \to  \dfrac{ {\rm d} \phi_{m,s}(t) }{ {\rm d}t } = -c \int_{C(t} = \partial S(t) }  {\rm d} \vec l ( \vec E + \frac{\vec v}{c} } \wedge \vec B} \\ = - c \int_{C(t) = \partial S'(t)} ( {\rm d} \vec l_{parallel} + {\rm d} \vec l_{\perp} ) $$