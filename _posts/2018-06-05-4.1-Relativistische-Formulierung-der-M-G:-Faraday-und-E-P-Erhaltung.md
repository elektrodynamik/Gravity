...
$$ \dfrac{ {\rm d} \phi_{m,s}(t) }{ {\rm d}t } = -c \int_{c'}  \frac{1}{\gamma} ( {\rm d} \vec l'_ {\parallel} + {\rm d} \vec l'_ {\perp} ) \cdot (\vec E_{\parallel} + \vec E_{\perp} + \frac{\vec}{c} \wedge \vec B_{\perp} ) \\ =  \int_{c'} ( {\rm d} \vec l'_ {\parallel} + {\rm d} \vec l'_ {\perp} ) $$