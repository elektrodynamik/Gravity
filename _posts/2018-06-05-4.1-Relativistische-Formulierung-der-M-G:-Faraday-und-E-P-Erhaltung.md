...
$$ ( T^{\mu \nu} )  \begin{\pmatrix} \dfrac{ \vec E^2 + \vec B^2}{8 \pi} & \dfrac{[\vec E \wedge \vec B ]^T}{4 \pi} \\  \dfrac{[\vec E \wedge \vec B ]}{4 \pi} & -(T_{ik} ) \end{pmatrix} =  \begin{pmatrix} \epsilon_{em} & c \vec g_{em}^T \\ c \vec g_em & - (T_{ik} )   $$ 
