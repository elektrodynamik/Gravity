...
$$ F_\alpha^{\ \beta} F_\beta^{ \ \gamma } F_\gamma^{ \ \delta}  F_\delta^{ \ alpha} = F^{\alpha \beta } F_{ \beta \gamma } F^{ \gamma \delta}  F_{\delta \alpha}  \\ =  2 (\vec E^2 -\vec B^2)^2 + 4 (\vec E \cdot \vec B )^2 $$
