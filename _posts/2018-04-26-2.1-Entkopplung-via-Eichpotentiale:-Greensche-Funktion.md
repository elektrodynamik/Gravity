...
$$ G = \dfrac{c}{2\pi^2} \int {\rm d}^3 k \dfrac{e^{i \vec k \cdot  \Delta \vec x} }{k} \sin(ck\Delta t) \\ $$ 
$$ = \dfrac{c}{\pi} \int {\rm d} k \sin(ck \Delta t) \int_{-1}^{-1} {\rm d} \cos \theta \underbrace{e^{i k \Delta x \cos \theta } }_{ \langle \dfrac{1}{i k \Delta x} e^{i k \Delta x \cos \theta } \rangle_{\cos \theta = -1}^{\cos(\theta) = 1} $$
