...
$$ \boxed{\phi_{\rm ret.} (\vec x,t) = \int {\rm d}^3 x' \dfrac{ \rho (\vec x, ) \delta(t' - t_{\rm ret.}) }{\mid\vec x - \vec x'\mid} \\ \vec A_{\rm ret.} (\vec x,t) = \int {\rm d}^3 x' {\rm d} t' \dfrac{ c \vec j (\vec x,t') \delta(t' - t_{\rm ret.}) }{\mid\vec x - \vec x'\mid} } $$ 
