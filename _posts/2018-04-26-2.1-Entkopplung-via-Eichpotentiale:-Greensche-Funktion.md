...
$$ G(\vec x,t, \vec x',t') = \dfrac{1}{(2\pi)^2} \int {\rm d}^3 k {\rm d}\omega \ \hat{G}(k,\omega) e^{ i [k(\vec x -\vec x') - \omega (t - t')] } \\ \delta(\vec x - \vec x') \ \delta(t - t') = \dfrac{1}{(2\pi)^4} \int {\rm d}^3 x {\rm d} t e^{ik(\vec x - \vec x') } e^{-i\omega(t - t')}$$

$$(\vec k^2 - \dfrac{\omega^2}{c^2}) \hat{G}(\vec k,\omega) = \dfrac{4\pi}{(2\pi)^2} \rightarrow \hat{G}(\vec k,\omega) = \dfrac{1}{\pi} (k^2 - \dfrac{\omega^2}{c^2})^{-1} \\ = \dfrac{1}{\pi} \dfrac{c}{(ck-\omega)(ck + \omega)} = \dfrac{1}{2\pi} \frac{c}{k} (\left \dfrac{1}{(\omega + ck)} - \dfrac{1}{ (\omega - ck)} \right ) $$ 
