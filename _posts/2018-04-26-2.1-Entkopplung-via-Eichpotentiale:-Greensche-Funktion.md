...
$$ \Rightarrow G(\vec x,t,\vec x',t') = \dfrac{c}{(2\pi)^3} \int {\rm d}^3 k d \int {\rm d} \omega \dfrac{e^{i\vec k \Delta \vec x} }{k} e^{-i\omega \Delta t} \cdot ( \dfrac{1}{(\omega + ck)} - \dfrac{1}{ (\omega - ck)} ) $$ 
