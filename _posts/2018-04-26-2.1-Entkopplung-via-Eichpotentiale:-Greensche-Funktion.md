...
$$ \boxed{\phi_{\rm ret.} (\vec x,t) = \int {\rm d}^3 x' \dfrac{ \rho (\vec x,t - \frac{\mid \vec x - \vec x'\mid}{c} )  }{\mid\vec x - \vec x'\mid} \\ \vec A_{\rm ret.} (\vec x,t) = \int {\rm d}^3 x' {\rm d} t' \dfrac{ \frac{1}{c} \vec j (\vec x',t - \frac{\mid\vec x - \vec x'\mid}{c}) }{\mid\vec x - \vec x'\mid} } $$ 
