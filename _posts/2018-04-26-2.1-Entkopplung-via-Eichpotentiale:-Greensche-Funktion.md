...
$$ \int {\rm d} \omega \ e^{-\overbrace{i \omega \Delta t}^{ >0 } } \cdot ( \dfrac{1}{(\omega + ck)} - \dfrac{1}{ (\omega - ck)} )\\ = -2 \pi i e^{-i\omega \Delta t} \mid_{\omega = -ck - i \varepsilon} + 2 \pi i e^{-i\omega \Delta t} \mid_{\omega = ck - i \varepsilon } \\ = -2\pi i e^{i c k \Delta t} + 2pi e^{-i c k \Delta t}$$
