...
$$ \dfrac{2c}{\pi \Delta x} \int_0^{\infty} {\rm d} k \sin(ck \Delta t) \sin(k \Delta x) \\ \stackrel{\lambda = ck}{=} \dfrac{1}{ \pi \Delta x} \int_{-\infty}^{\infty} {\rm d} \lamda \sin(\lambda \Delta t) \sin(\lambda \dfrac{\Delta x}{c}) \\ \stackrel{Trig.}{=} \dfrac{1}{2\pi \Delta x}  \int_{-\infty}^{\infty} {\rm d} \lambda \{ \cos ( \lambda [ \Delta t - \dfrac{\Delta x}{c}] ) $$ 
