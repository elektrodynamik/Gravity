...
$$ f(\vec x,t) = \dfrac{1}{(2 \pi)^2} \int_{\mathbb{R}^4} {\rm d} \omega \d^3 k \hat{f} (\vec k,\omega) e^{i(\vec k \vec x - \omega t)} \\ \hat{f}(\vec k,\omega) = \dfrac{1}{(2 \pi)^2} \int_{\mathbb{R}^4} {\rm d} \t {\rm d}^3 x \hat{f} (\vec x,t) e^{-i\vec k \vec x - \omega t)} $$
