...
Allgemeine Lösung (mit beliebigen $$ \hat{f_+},\hat{f_-} $$) ist mit $$ \omega(\vec k) = c \mid k\mid = ck \\ f_{\rm hom.} (\vec x,t) = \dfrac{1}{(2 \pi)^2} \int {\rm d}^3 k \{ \hat{f_+}(\vec k) e^{i(\vec k \vec x - \omega(\vec k) t } \}$$
