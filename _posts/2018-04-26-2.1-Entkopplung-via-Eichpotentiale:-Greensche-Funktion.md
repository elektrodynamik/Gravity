...
$$ \rightarrow (-\partial_x^2 f+ \partial_{ct}^2 f) (\vec x,ct) = 0 \\ f(\vec x, 0) = \frac{1}{2} [ u(x) + u(x) ] + \int_{x -0}^{x+0} \dots {\rm d} \bar{x} = u(x) \\ \partial_{ct} f(\vec x,0) = \frac{1}{2} [ u(x+ct) + u(x-ct) ] $$
