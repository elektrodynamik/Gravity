...
$$ \boxed{ \text{Greens-Funktion}  \ G / \text{Fundamentallösung} \ \delta \\ f(\vec x,t) = \int {\rm d}^3 x' {\rm d} t' G(\vec x ,t,\vec x',t') g(\vec x',t')  \\ \text{Bsp.} \ \square-\text{Operator} : \square G(\vec x,t,\vec x',t') = (-\Delta + \partial_{ct}^2) G(\dots) = 4 \pi \delta (\vec x - \vec x') \delta (t - t') }$$

**Lösung mit Fouriertransformation**

$$ G(\vec x,t, \vec x',t') = \dfrac{1}{2\pi}^2 \int {\rm d}^3 k {\rm d}\omega \ \hat{G}(k,\omega) e^{ i [k(\vec x -\vec x') - \omega (t - t')] } \\ \delta(\vec x - \vec x') \ \delta(t - t') =  $$
