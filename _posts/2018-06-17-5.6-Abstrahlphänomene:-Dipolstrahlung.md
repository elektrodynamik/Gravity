...
Sei nun $$ r = \mid \vec x \mid $$.
Durch Multiplikation mit der Exponentialfunktion erhält man:

$$ \dfrac{ e^{ i k\mid \vec x - \vec '\mid } }{\mid \vec x - \vec x' } \approx e^{i k r} e^{ik \frac{\vec x \vec x'}{r}} r^{-1} (1 + \frac{\vec x \vec x'}{r^2} ) = \dfrac{e^{ikr}}{r} e^{- ik \vec n \vec x'} ( 1 + dfrac{ \vec n \vec x'}{\vec n \vec x } ) = e^{ikr}$$ 
