...
- Mit $$a^\mu u_\mu = 0;  (F^\mu) = ( F^0, \gamma(v) \vec F)^T $$ folgt; $$ 0 = m a^\mu u_\mu = F^\mu u_\mu = \gamma (F^0 c - \gamma \vec F \vec v) \Rightarrow \dfrac{\rm d \p^0}{rm d \tau} = F^0 = \frac{\gamma}{c} \vec F \vec v = \frac{\gamma}{c} \dfrac{\rm d \vec F \vec d \vec x}{rm d \t} = \frac{\gamma}{c} \dfrac{\rm d W}{\rm d t} = \frac{1}{c} \dfrac{\rm d W}{d \tau}. $$ mit $$dW $$ die Arbeit, die $\vec F $$ über $$ d \vec x$$ geleistet hat. $$ \to  \dfrac{\rm d W}{rm d \tau} = \dfrac{d}{d \tau} ( c p^0) = \dfrac{d}{d \tau} (\gamma (v) m c^2}$$