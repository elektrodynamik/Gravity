...
Idee: 'Bildladung'  $$ \tilde{q}$$ in der Kugel d.h. ($$\mathbb{R}^3 \B(\vec 0,R) := V) $$ in $$b \vec e_1 $$ mit $$ \vec x = x \vec n :$$ 

$$ \rightarrow  \phi(\vec x) = G + g = \dfrac{q}{\mid x \vec n - a\vec e_x\mid} + \dfrac{\tilde{q}}{\mid x \vec n - b \vec e_x $$