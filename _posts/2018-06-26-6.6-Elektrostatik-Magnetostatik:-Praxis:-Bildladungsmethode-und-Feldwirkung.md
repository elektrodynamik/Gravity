...
$$ \cos \alpha = \sin \theta \sin \theta' \cos(\phi - \phi') + \cos \theta \cos \theta', \\ \dfrac{1}{\mid \vec x -\vec x'} = \frac{1}{r} ( 1 - 2 \frac{r'}{r} \cos \alpha + (\frac{r'}{r})^2)^{-\frac{1}{2}} \\ = \frac{1}{r} \sum_0^\infty (\frac{r'}{r})^l P_l (\cos(\alpha)) = \sum_{l = 0}$$