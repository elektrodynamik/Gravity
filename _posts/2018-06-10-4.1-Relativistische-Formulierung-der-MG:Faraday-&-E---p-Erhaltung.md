...
$$ \dfrac{{\rm d} \phi_{m,s}(t)}{{\rm d}t} = \int_{S(t)} {\rm d} \vec \sigma \partial_t \vec B + \int_{S(t)} \frac{{\rm d} \sigma }{{\rm d} t} \cdot \vec B \stackrel{II}{=} - c \int (\vec nabla \wedge \vec E ) + \int \frac{ {rm d} \vec \sigma}{{\rm d} t }$$ 