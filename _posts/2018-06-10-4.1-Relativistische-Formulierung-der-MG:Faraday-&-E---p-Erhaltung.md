...
$$ \Rightarrow \vec E(\vec x,t) = -\nabla \phi - \partial_{ct} \vec A  = \frac{\gamma q}{(( \gamma^2((x - v t^2) + y^2 +z^2))^{\frac{3}{2}} } \begin{pmatrix} x - v t \\ y \\ z $$

$$ \Rightarrow \vec B (\vec x , t} = \vec \nabla \wedge \vec A(\vec x,t} = \frac{\beta \gamma q }{[ \dots ]^^{\frac{3}{2}} \begin{pamtrix} 0 \\ z \\ -y \end{pmatrix} $$ 

- Verallgemeinertes Induktionsgesetz

Faraday IG: $$ \int_{C = \partial S} \rm{d}\vec l \cdot \vec E(\vec x,t} = - \int_{S} \rm{\sigma} \partial_{ct} \vec B(\vec x,t) $$ 
